...
output "github_actions_plan_role_arn" {
  ...
  value       = aws_iam_role.github_actions_deploy_plan.arn
}
output "github_actions_apply_role_arn" {
  description = "IAM role ARN to use in GitHub Actions with aws-actions/configure-aws-credentials."
  value       = aws_iam_role.github_actions_deploy_apply.arn
}
...
output "github_actions_environment_variables" {
  ...
  value = {
    ...
    AWS_PLAN_ROLE_TO_ASSUME   = aws_iam_role.github_actions_deploy_plan.arn
    AWS_APPLY_ROLE_TO_ASSUME  = aws_iam_role.github_actions_deploy_apply.arn
    AWS_ACCOUNT_ID            = data.aws_caller_identity.current.account_id
    GITHUB_REPOSITORY         = "${var.github_owner}/${var.github_repo}"
    GITHUB_ALLOWED_REFS_PLAN  = var.allowed_refs_plan
    GITHUB_ALLOWED_REFS_APPLY = var.allowed_refs_apply
  }
}
