...
```text
repo:dantejauregui/aws_vpcHA_privateALB_FastAPI_ecsFargate_RDS_OpenTelemetry:ref:refs/heads/main
```
...
That is acceptable for bootstrap and learning velocity. For production, we are using `managed_policy_arn_plan` and `managed_policy_arn_apply` with a narrower Terraform deployment policy.
...
```bash
AWS_PROFILE=infra-dev-admin terraform init -reconfigure -backend-config="profile=infra-dev-admin"
```
